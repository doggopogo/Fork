package com.example.fitnessandroid.homescreen.sleep.user
...
@Composable
fun UserSleepScreen(userInfo : UserInfo) {
    ...
    val currentUserSleep : MutableState<UserSleep?> = remember{ mutableStateOf(null)}
    ...
    Column(
        ...
        if (openDialogList.value) {
            ShowListModal(setShowModal = {openDialogList.value = false}) {
                ShowRows(userSleep = userInfo.userSleep.value, 36500) {
                    currentUserSleep.value = it
        ...
        if (openDialogEdit.value) {
            ModuleModal(
                ...
                "Sommeil",
                R.color.sleep ,
                { UserSleepModal(userSleep = currentUserSleep.value) }
            )
            {
        ...
        Graph(userSleep = userInfo.userSleep.value, nbOfDay.value)

        SelectorPeriodOfTime(color = colorResource(id = R.color.sleep)) {
            nbOfDay.value = it
        }
        ...
        Column(
            ...
            ShowRows(userSleep = userInfo.userSleep.value, nbOfDay.value){
                currentUserSleep.value = it
                openDialogEdit.value = true
            }
        }
        ...
        Row(
            ...
            Button(
                onClick = { userInfo.userSleep.value.isNotEmpty().let { openDialogList.value = true } },
...
@Composable
private fun Graph(userSleep :  List<UserSleep>, nbOfDay: Int) {
    Column(
        modifier = Modifier
            .fillMaxHeight(0.4f)
            .fillMaxWidth()
        ...
        val filteredListUserSleep : List<UserSleep> = userSleep.filter {
            it.endSleep.isInLastXDays(nbOfDay)
        }.sortedByDescending { it.endSleep }

        Log.d("nemo filteredListUserSleep", filteredListUserSleep.toString())

        val tempsPoints = (0 until nbOfDay).map { 0f }.toMutableList()
        tempsPoints.apply {
            filteredListUserSleep.forEach {
                this[it.endSleep.nbOfDaySinceToday()] += it.totalSleepTime.toFloat()
            }
        }
...
@Composable
private fun ShowRows(userSleep : List<UserSleep>, nbOfDay : Int, editModal:(UserSleep) -> Unit) {
    val rowsDataUiState = mutableListOf<RowDataUiState>()

    userSleep.filter {
        it.endSleep.isInLastXDays(nbOfDay)
    }.sortedByDescending { it.endSleep }.map{
        rowsDataUiState.add(
            RowDataUiState(
                title = it.endSleep.toString(),
                edit = { editModal(it) },
                delete = { delete(it) },
...
private fun delete(userSleep: UserSleep) {
    FitHabData.deleteUserSleep(userSleep.idUserSleep)
}

@Composable
private fun RowContent(userSleep: UserSleep) {
    Column(
        modifier = Modifier.fillMaxWidth()
    ) {
        Text(text = "Nombre de réveils : " + userSleep.numberOfAwakening)
        Text(text = "Heures de sommeil : " + userSleep.totalSleepTime + "h")
        Text(text = "État au réveil : " + Utilities.convertToShowMindset(userSleep.mindset))
    }
}
...
private fun RowContentPreview() {
    PreviewComposable(backgroundColor = Color.White) {
        RowContent(Utilities.userInfoForPreview().userSleep.value.first())
    }
}

@Preview(showSystemUi = true)
@Composable
private fun UserSleepScreenPreview() {
    UserSleepScreen(userInfo = Utilities.userInfoForPreview())
}